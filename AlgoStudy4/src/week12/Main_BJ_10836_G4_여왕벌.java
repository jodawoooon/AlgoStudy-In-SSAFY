...
/*
	크기가 M*M 벌집, 각 칸에 애벌레 1마리
	첫날 아침 모든 애벌레들의 크기는 1이고, 이러한 과정을 N일 동안 반복
	커지는 정도 -> +0, +1, +2 중 하나
	
	1. 0행, 0열은 자신이 자라는 정도를 스스로 결정
		입력으로 주어질 것 (왼쪽 제일 아래 칸에서 시작하여 위쪽으로 가면서 읽고, 제일 위쪽 칸에 도착하면 오른쪽으로 가면서 행의 끝까지 읽었다)
	2. 나머지 애벌레들은 자신의 왼쪽(L), 왼쪽 위(D), 위쪽(U)의 애벌레들이 다 자란 다음, 
